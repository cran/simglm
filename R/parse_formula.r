...
parse_formula <- function(sim_args) {
  if (is.list(sim_args[['formula']])) {
    ...
    fixed <- as.formula(paste0(
      ...
      gsub(
        ...
        gsub(
          "\\+\\s*(\\s+|\\++)\\(.*?\\)",
          "",
          as.character(sim_args[['formula']])[3]
        )
      )
    ))
    ...
    list(outcome = outcome, fixed = fixed, randomeffect = randomeffect)
  }
}
...
parse_formula_list <- function(sim_args) {
  lapply(seq_along(sim_args[['formula']]), function(xx) {
    list(
      outcome = as.character(sim_args[['formula']][[xx]])[2],
      fixed = as.formula(paste0(
        "~",
        gsub(
          "^\\s+|\\s+$",
          ...
          gsub(
            "\\+\\s*(\\s+|\\++)\\(.*?\\)",
            "",
            as.character(sim_args[['formula']][[xx]])[3]
          )
        )
      )),
      randomeffect = gsub(
...
parse_randomeffect <- function(formula) {
  cluster_id_vars <- gsub(
    "^\\s+|\\s+$",
    "",
    gsub(
      "\\)",
      "",
      unlist(lapply(seq_along(formula), function(xx) {
        strsplit(formula, "\\|")[[xx]][2]
      }))
    )
  )
...
parse_multiplemember <- function(sim_args, random_formula_parsed) {
  multiple_member_re <- lapply(
    seq_along(sim_args[['randomeffect']]),
    function(xx) {
      sim_args[['randomeffect']][[xx]][['multiple_member']]
    }
  )
  multiple_member_re <- unlist(lapply(
    seq_along(multiple_member_re),
    function(xx) {
      !is.null(multiple_member_re[[xx]])
    }
  ))
  num_res <- lapply(
    lapply(seq_along(random_formula_parsed[['random_effects']]), function(xx) {
  ...
  list(
    multiple_member_idvars = multiple_member_idvars,
    num_res = num_res,
...
parse_power <- function(sim_args, samp_size) {
  ...
  if (is.list(reg_weights) && !is.null(names(reg_weights)) && all(names(reg_weights) != "")) {
    if (is.null(sim_args[['model_fit']][['formula']])) {
      power_formula <- sim_args[['formula']]
    } else {
      power_formula <- sim_args[['model_fit']][['formula']]
    }
    power_fixed_formula <- parse_formula(list(formula = power_formula))[['fixed']]
    reg_weights <- regression_weight_terms(reg_weights, power_fixed_formula)
  }
  ...
  if (is.null(sim_args[['power']][['direction']])) {
    ...
    number_tails <- ifelse(
      sim_args[['power']][['direction']] %in% c('lower', 'upper') == TRUE,
      1,
      2
    )
  ...
  test_statistic <- lapply(seq_along(stat_dist), function(ii) {
    if (
      is.null(sim_args[['power']][['opts']][['df']]) & stat_dist[ii] == 'qt'
    ) {
  ...
  power_out <- lapply(seq_along(test_statistic), function(xx) {
    list(
      test_statistic = test_statistic[[xx]],
      alpha = alpha[xx],
  ...
  names(power_out) <- names(reg_weights)
...
parse_varyarguments <- function(sim_args) {
  ...
  if (any(sapply(conditions, is.list))) {
    ...
    for (tt in seq_along(list_conditions)) {
      names(list_conditions[[tt]]) <- gsub(
        "[0-9]*",
        "",
        names(list_conditions[[tt]])
      )
    }
    lapply(1:nrow(conditions), function(xx) {
      c(
        sim_args,
        simp_conditions[xx, , drop = FALSE],
        do.call(
          'c',
          lapply(seq_along(list_conditions), function(tt) {
    ...
    lapply(1:nrow(conditions), function(xx) {
      c(sim_args, conditions[xx, , drop = FALSE])
    })
  }
}
...
parse_varyarguments_w <- function(sim_args, name) {
  conditions <- expand.grid(
    list_select(sim_args[['vary_arguments']], names = name, exclude = FALSE),
    KEEP.OUT.ATTRS = FALSE
  )
  if (any(sapply(conditions, is.list))) {
    loc <- sapply(conditions, is.list)
...
parse_correlation <- function(sim_args) {
  ...
  if (!is.null(sim_args[['correlate']][['fixed']])) {
    if (length(fixed_vars) != nrow(sim_args[['correlate']][['fixed']])) {
      pairwise_options <- gtools::combinations(
        length(fixed_vars),
        2,
        fixed_vars
      )
  ...
  fixed_correlation <- dataframe2matrix(
    sim_args[['correlate']][['fixed']],
    corr_variable = 'corr',
    var_names = c('x', 'y')
  )

  random_correlation <- dataframe2matrix(
    sim_args[['correlate']][['random']],
    corr_variable = 'corr',
    var_names = c('x', 'y')
  )
